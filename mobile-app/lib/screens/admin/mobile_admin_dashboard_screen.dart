...
class _MobileAdminDashboardScreenState extends State<MobileAdminDashboardScreen> {
  ...
  Future<void> _handleAdminFinalReview(Map<String, dynamic> item, bool isApproved) async {
    ...
    final notifDesc = isApproved
        ? 'Great news! Admin approved your video recording "${item['title']}"!'
    ...
    if (kIsWeb) {
      try {
        ...
        final rawSubmissions = web.localStorageGet('platform_qc_submissions');
        if (rawSubmissions != null) {
          ...
          web.localStorageSet('platform_qc_submissions', jsonEncode(subs));
        }

        final bc = web.BroadcastChannelStub('platform_realtime_channel');
        bc.postMessage(jsonEncode({'type': 'ADMIN_REVIEW_COMPLETED', 'status': newStatus, 'videoId': videoId}));
        bc.close();
      } catch (_) {}
    }
  ...
  void _connectRealtimeStream() {
    if (kIsWeb) {
      try {
        final eventSource = web.EventSourceStub('$_apiBaseUrl/notifications/stream');
  ...
  void _triggerDownload(String downloadUrl) {
    if (kIsWeb) {
      try {
        web.windowOpen(downloadUrl, '_blank');
      } catch (e) {
