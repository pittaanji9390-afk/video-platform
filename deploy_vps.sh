...
# 4. Create production .env (interactive - you must provide real values)
echo "[4/6] Creating production environment..."
if [ ! -f /var/www/video_app/backend/.env ]; then
    echo "Creating .env template - YOU MUST EDIT THIS FILE WITH REAL SECRETS!"
...
JWT_REFRESH_SECRET=CHANGE_ME_TO_ANOTHER_RANDOM_64_CHAR_STRING
JWT_EXPIRES_IN=15m
JWT_REFRESH_EXPIRES_IN=7d
CORS_ORIGIN=https://elevateiq-softtech.com
ENVEOF
    echo "WARNING: Edit /var/www/video_app/backend/.env with real production secrets!"
    echo "  Run: nano /var/www/video_app/backend/.env"
else
