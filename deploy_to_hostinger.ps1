...
if (-not $VPS_USER) {
    $VPS_USER = Read-Host "Enter VPS username (default: root)"
}
$VPS_PASS = Read-Host "Enter VPS password" -AsSecureString
...
$SecurePass = ConvertTo-SecureString $VPS_PASS_PLAIN -AsPlainText -Force
...
Write-Host "NOTE: Edit the .env file on the VPS with your actual production secrets!" -ForegroundColor Yellow
$envContent = @'
NODE_ENV=production
PORT=5000
DB_HOST=postgres
